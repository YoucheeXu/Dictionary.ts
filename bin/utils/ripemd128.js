...
function padandsplit(message) {
    ...
    */
    ...
    bufOfPad[0] = 0x80;
    let msgPad = (0, utils_1.BufferConcat)(message, bufOfPad);
    ...
    let newMsg = (0, utils_1.BufferConcat)(msgPad, end);
    ...
    for (let i = 0; i < newMsg.length; i += 64) {
        for (let j = 0; j < 64; j += 4) {
            let value = (0, utils_1.Bytes2Num)("<L", newMsg.slice(i + j, i + j + 4));
            X[i / 64][j / 4] = value;
        }
    }
    return X;
}
function add(firstNum, ...args) {
    let sum = firstNum;
    for (let num of args) {
        sum += num;
    }
