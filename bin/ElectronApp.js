...
const WordsDict_1 = require("./components/WordsDict");
const GDictBase_1 = require("./components/GDictBase");
const SDictBase_1 = require("./components/SDictBase");
const MDictBase_1 = require("./components/MDictBase");
const AuidoArchive_1 = require("./components/AuidoArchive");
const DownloardQueue_1 = require("./utils/DownloardQueue");
class ElectronApp {
    constructor(_startPath) {
    ...
    async AddDictBase(name, dictSrc, format, download = null) {
        let dictBase;
        if (format.Type == 'ZIP') {
    ...
    async TriggerDownload(owner, word, localFile) {
            ...
            while (r = iterator.next(), !r.done) {
                ...
                if (download.Dict == dict.szName) {
                    ...
                    if (ret == 1) {
                        ...
                        if (match) {
                            let url = match[0];
                            this._dQueue.AddQueue(word, url, localFile, (dFile) => {
    ...
    async Record2File(file, something) {
        return new Promise((resolve, reject) => {
            fs.writeFile(file, something, { 'flag': 'a' }, (err) => {
                if (err) {
    ...
    SaveConfigure() {
        return new Promise((resolve, reject) => {
            // Indent by 4 spaces
            fs.writeFile(this._cfgFile, JSON.stringify(this._cfg, null, 4), { 'flag': 'w' }, (err) => {
                if (err) {
    ...
    async Close() {
        this._dictMap.forEach(async (dict, tabId) => {
