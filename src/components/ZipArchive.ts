...
export class ZipArchive {
    ...
    private fileList: string[] = new Array();

    constructor(readonly zipFile: string, readonly compression: string = "", readonly compresslevel: string = "") {
        // this.zip = zipFile;
    ...
    public async readFileAsync(fileName: string): Promise<[boolean, Buffer]> {
        return new Promise<[boolean, Buffer]>((resolve, reject) => {
            this.zip.file(fileName)!.async("nodebuffer").then(
                (datum) => {
                    resolve([true, datum]);
                },
                (reason) => {
